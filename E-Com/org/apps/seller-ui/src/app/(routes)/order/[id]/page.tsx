...
const OrderDetailsPOage = () => {
  ...
  const handleStatusChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setUpdating(true)
    try {
      await axiosInstance.put(`/order/api/update-status/${orderId}`,{deliveryStatus:newStatus})
      setOrder((prev:any)=>({...prev,deleveryStatus:newStatus}))
    } catch (error) {
  ...
  if (!order) {
    return <p className='text-center text-sm txt-red-500'>Order not Found</p>
  }
  return (<div className='max-w-5xl mx-auto px-4 py-10'>
    ...
    <h1 className='text-2xl font-bold text-gray-200 md-4'>
...
export default OrderDetailsPOage
